...
from diatomic import log_time # 假设你保留了这个装饰器，如果没有可以去掉
...
# 3. 哈密顿量构建函数
# ==========================================
def get_quantum_numbers_from_index(idx, dims):
    """
    辅助函数：将线性索引转换为量子数索引 (i_S1, i_I1, i_S2, i_I2)
    dims = (dim_S1, dim_I1, dim_S2, dim_I2)
    """
    d_S1, d_I1, d_S2, d_I2 = dims
    
    # 假设 kron 顺序: S1 (x) I1 (x) S2 (x) I2
    # idx = i_S1 * (d_I1*d_S2*d_I2) + i_I1 * (d_S2*d_I2) + i_S2 * (d_I2) + i_I2
    ...
    return i_S1, i_I1, i_S2, i_I2

def get_index_from_quantum_numbers(indices, dims):
    i_S1, i_I1, i_S2, i_I2 = indices
    d_S1, d_I1, d_S2, d_I2 = dims
    return i_S1 * (d_I1*d_S2*d_I2) + i_I1 * (d_S2*d_I2) + i_S2 * (d_I2) + i_I2
def build_symmetrization_matrix_for_L(basis, L):
    """
    针对特定的角动量 L，构建自旋空间的投影矩阵 P_spin。
    """
    sys = basis.sys
    if not getattr(sys, 'is_homonuclear', False):
        return np.eye(basis.dim_spin) # 异核直接返回单位阵

    # 1. 确定自旋对称性要求
    # 玻色子: eta=+1, 费米子: eta=-1
    # eta = (-1)^L * epsilon_spin
    eta = 1 if sys.statistics == 'boson' else -1
    parity_L = (-1)**L
    epsilon_spin = eta * parity_L 

    dims = (basis.dim_S1, basis.dim_I1, basis.dim_S2, basis.dim_I2)
    processed = np.zeros(basis.dim_spin, dtype=bool)
    projection_vectors = []

    for idx in range(basis.dim_spin):
        if processed[idx]: continue
            
        i_S1, i_I1, i_S2, i_I2 = get_quantum_numbers_from_index(idx, dims)
        
        # 交换粒子 1 和 2
        swapped_indices = (i_S2, i_I2, i_S1, i_I1)
        swapped_idx = get_index_from_quantum_numbers(swapped_indices, dims)
        
        vec = np.zeros(basis.dim_spin)
        
        if idx == swapped_idx: # 对角态 (如 |a, a>)
            if epsilon_spin == 1: # 只有对称要求下存在
                vec[idx] = 1.0
            ...
            norm = 1.0 / np.sqrt(2)
            if epsilon_spin == 1:
                vec[idx] = norm; vec[swapped_idx] = norm
            else:
                vec[idx] = norm; vec[swapped_idx] = -norm
            projection_vectors.append(vec)
            processed[idx] = True
            processed[swapped_idx] = True
            
    if not projection_vectors:
        return np.zeros((basis.dim_spin, 0))
        
    return np.column_stack(projection_vectors)
def build_symmetrization_matrix(basis: CollisionBasis, L: int):
    """
    构建对称化投影矩阵 P.
    H_sym = P.T @ H_full @ P
    ...
    Returns:
        P: (dim_full_spin, dim_sym_spin) 的矩阵
    """
    sys = basis.sys
    
    # 1. 如果是异核分子，不需要对称化，返回单位矩阵
    if not sys.is_homonuclear:
        return np.eye(basis.dim_spin)
    
    # 2. 确定需要的自旋对称性 epsilon_spin
    # 总对称性 eta: Boson=+1, Fermion=-1
    # eta = (-1)^L * epsilon_spin
    # epsilon_spin = eta * (-1)^L
    
    eta = 1 if sys.statistics == 'boson' else -1
    parity_L = (-1)**L
    epsilon_spin = eta * parity_L # +1 (Symmetric) or -1 (Antisymmetric)
    
    # 3. 遍历基矢，寻找配对
    # dims = (dim_S1, dim_I1, dim_S2, dim_I2)
    # 注意: 同核原子 dim_S1==dim_S2, dim_I1==dim_I2
    dims = (basis.dim_S1, basis.dim_I1, basis.dim_S2, basis.dim_I2)
    ...
    for idx in range(basis.dim_spin):
        ...
        # 找到交换粒子 1 和 2 后的态的索引
        # Swapped: (S2, I2, S1, I1) -> (i_S2, i_I2, i_S1, i_I1)
        swapped_indices = (i_S2, i_I2, i_S1, i_I1)
        swapped_idx = get_index_from_quantum_numbers(swapped_indices, dims)
        ...
        if idx == swapped_idx:
            # 对角态 (例如 |up, up, up, up>)
            # 只有当我们需要对称自旋态 (epsilon=+1) 时，这个态才存在
            # 且它本身就是对称的
            if epsilon_spin == 1:
                vec[idx] = 1.0
                projection_vectors.append(vec)
            ...
            if epsilon_spin == 1:
                # 对称态: (|ab> + |ba>) / sqrt(2)
                vec[idx] = 1.0 / np.sqrt(2)
                vec[swapped_idx] = 1.0 / np.sqrt(2)
                projection_vectors.append(vec)
            else:
                # 反对称态: (|ab> - |ba>) / sqrt(2)
                vec[idx] = 1.0 / np.sqrt(2)
    ...
    # 将列向量堆叠成矩阵 P
    if len(projection_vectors) == 0:
        # 极端情况 (例如 fermions 只有 1 个态且要求反对称?)
        return np.zeros((basis.dim_spin, 0))
        
    P = np.column_stack(projection_vectors)
    return P
def build_total_projection_matrix(basis):
    """
    构建总投影矩阵 P_total (包含所有 L 块)。
    返回 P，形状为 (Dim_Full, Dim_Sym)
    """
    P_blocks = []
    for L in basis.L_list:
        # 获取自旋部分的投影
        P_spin = build_symmetrization_matrix_for_L(basis, L)
        
        # 扩展到空间部分 (Lz)
        dim_L_space = int(2*L + 1)
        # P_L = P_spin (x) Identity_L
        P_L = np.kron(P_spin, np.eye(dim_L_space))
        P_blocks.append(P_L)
